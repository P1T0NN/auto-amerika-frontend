...
<div class="space-y-4">
    <Label class="text-base font-medium">
        {m["EditCarPage.EditCarTabSettings.MediaManagement.additionallyAddedImages"]()} ({editCarImagesContext.newImages.length})
        <span class="text-green-600 text-sm font-normal">-  {m["EditCarPage.EditCarTabSettings.MediaManagement.forAdding"]()} </span>
    </Label>
            ...
            <div class="relative group">
                <div class="aspect-square overflow-hidden rounded-lg border-green-200 border-2">
                    <img
                        ...
                        alt={m["EditCarPage.EditCarTabSettings.MediaManagement.newImage"]()}
                ...
                <button
                    ...
                    aria-label={m["EditCarPage.EditCarTabSettings.MediaManagement.removeNewImage"]()}
                >
                    <X class="h-4 w-4" />
                </button>

                <div class="absolute bottom-0 left-0 right-0 bg-green-500 bg-opacity-75 text-white text-xs p-2 rounded-b-lg">
                    {m["EditCarPage.EditCarTabSettings.MediaManagement.newImage"]()}: {image.name}
                </div>
            </div>
        {/each}
    </div>
